...
template <typename Dtype>
int ImagePairDataLayer<Dtype>::myrandomdata (int i) { return caffe_rng_rand()%i;}
...
void ImagePairDataLayer<Dtype>::ShuffleImages() {
  ...
  vector<int> tnum;
  int pairsize = this->layer_param_.image_pair_data_param().pair_size();

  for(int i = 0; i < num_images / pairsize; i ++)
  {
    tnum.push_back(i);
  }
  std::random_shuffle(tnum.begin(), tnum.end(), ImagePairDataLayer<Dtype>::myrandomdata);
  tlines_.clear();
  for(int i = 0; i < num_images / pairsize; i ++)
  {
